...
CLASSES_PATH = r'D:/Fishy/server/Yolo/class_vie.txt'         # file tiếng Việt (dùng hiển thị)
CLASSES_CODE_PATH = r'D:/Fishy/server/Yolo/class_code.txt'   # ✅ thêm: file code (dùng train)
...
async def lifespan(app: FastAPI):
    ...
    # 3. Load tên class + mapping code -> vie (hạn chế sửa code nhất)
    #    - classes_code.txt: DP.135, P.102, ...
    #    - classes_vie.txt : Hết tất cả các lệnh cấm, Cấm đi ngược chiều, ...
    #    => Mapping theo THỨ TỰ DÒNG (cls_id)
    try:
        if not os.path.exists(CLASSES_PATH):
            logger.warning("Không tìm thấy file class tiếng Việt.")
        if not os.path.exists(CLASSES_CODE_PATH):
            logger.warning("Không tìm thấy file class code.")

        if os.path.exists(CLASSES_PATH) and os.path.exists(CLASSES_CODE_PATH):
            with open(CLASSES_PATH, "r", encoding="utf-8") as f:
                vie_lines = [ln.strip() for ln in f if ln.strip()]
            with open(CLASSES_CODE_PATH, "r", encoding="utf-8") as f:
                code_lines = [ln.strip() for ln in f if ln.strip()]

            if len(code_lines) != len(vie_lines):
                logger.warning(
                    f"⚠️ Mismatch số dòng: code={len(code_lines)} vie={len(vie_lines)}. "
                    "Mapping có thể sai (2 file phải cùng thứ tự & cùng số dòng)."
                )

            n = min(len(code_lines), len(vie_lines))

            # ✅ Chỉ cần đổ đúng dict này là detect() / detect-lite() chạy như cũ
            # Option A: chỉ tiếng Việt (báo kết quả thuần Việt)
            # vn_classes_dict = {i: vie_lines[i] for i in range(n)}

            # Option B: kèm code để dễ debug/đối chiếu (khuyên dùng)
            vn_classes_dict = {i: f"{code_lines[i]} - {vie_lines[i]}" for i in range(n)}

            logger.info(f"Đã mapping {n} class (code -> vie). Ví dụ: 0='{vn_classes_dict.get(0)}'")

        elif os.path.exists(CLASSES_PATH):
            # fallback: chỉ có file vie thì dùng như cũ
            with open(CLASSES_PATH, 'r', encoding='utf-8') as f:
                lines = [line.strip() for line in f.readlines() if line.strip()]
                vn_classes_dict = {i: name for i, name in enumerate(lines)}
            logger.info(f"Đã tải {len(vn_classes_dict)} tên biển báo (vie-only).")

    except Exception as e:
        logger.error(f"Lỗi đọc/mapping file class: {e}")
