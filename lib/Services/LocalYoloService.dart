...
class LocalYoloService {
  ...
  Future<void> init() async {
    ...
    if (!supportsOnDevice) {
      debugPrint('LocalYoloService: platform hiện tại không bật YOLO on-device.');
      return;
    }
    ...
    try {
      ...
      if (shape.length != 4) {
        throw Exception('Input tensor không hợp lệ: $shape');
      }
  ...
  Future<YoloLiteResponse> detectCameraFrame(
    ...
    if (converted == null) {
      return YoloLiteResponse(
        summaryText: 'Không đổi được frame camera',
        boxes: const [],
        width: 0,
        height: 0,
      );
  ...
  List<YoloBox> _decodeOutput(
    ...
    if (shape.length != 3 || shape[0] != 1) {
      debugPrint('Output shape chưa được hỗ trợ: $shape');
      return const [];
    }
    ...
    if (!treated) {
      debugPrint('Không suy ra được orientation của output: $shape');
    }
  ...
  String _buildSummary(List<YoloBox> boxes) {
    if (boxes.isEmpty) return 'Không phát hiện biển báo';
